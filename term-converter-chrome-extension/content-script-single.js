...
Panel.prototype.create = function () {
    ...
    let html = `
        <!--X是用来做关闭按钮-->
        <header>Transfer<span class="close">X</span></header>
  <main>
    <div class="source">
      <div class="title">Original Term</div>
      <!--这里动态插入用户选中的需要翻译的内容 所以先留空 什么都不写-->
      <div class="content"></div>
    </div>
    <div class="dest">
      <div class="title">Matched Terms</div>
...
Panel.prototype.translate = function (raw) {
    ...
    let tlValue = 'Artificial Intelligence' // 转换成的术语领域
    // })

    //查看用户是否已经设置了语言类型
    chrome.storage.sync.get(['tl'], (result) => {

        if (result.tl) {
            tlValue = result.tl.value
            this.container.querySelector('.dest .title').innerText = result.tl.key // 显示翻译出来的词语的领域
        }
        
        let prompt = raw + " =>" // Form prompt

        const data =  { model: 'davinci:ft-personal-2023-06-10-08-03-27', 
                        prompt: prompt,
                        max_tokens: 3,
                        temperature: 0
                    }
        fetch(`https://api.openai.com/v1/completions`, {
            ...
            headers: {
              ...
              "Authorization": "Bearer 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",
            },
            //body: `model=davinci:ft-personal-2023-06-10-08-03-27&prompt=${prompt}&max_tokens=5&temperature=0`,
            body: JSON.stringify(data) 
            }
        )
            .then(res => res.json())
            .then(res => {
                //异步完成后,把获取到的已翻译完成的译文内容插入到翻译面板
                this.dest.innerText = res['choices'][0]['text']
            })
    })
}
